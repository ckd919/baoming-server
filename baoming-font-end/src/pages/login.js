/**
 * 登录页 — 三模式：管理员登录 / 管理员注册 / 用户登录
 */
import { showToast } from '../utils.js';
import { adminLogin, adminRegister, userPhoneLogin, isLoggedIn, isAdmin, isUser } from '../store.js';
...
export function renderLogin() {
  ...
  app.innerHTML = `
    <div class="login-page">
      ...
      <div class="login-tabs">
        <button class="login-tab active" data-mode="admin" id="tab-admin">🔑 管理员</button>
        <button class="login-tab" data-mode="user" id="tab-user">📱 用户登录</button>
      </div>

      <!-- ========== 管理员登录表单 ========== -->
      <div class="login-card" id="admin-login-form">
        ...
        <div class="text-center mt-12">
          <span class="text-link" id="switch-to-register" style="font-size:13px;color:var(--primary);cursor:pointer">
            没有账号？注册管理员
          </span>
        </div>
      </div>
      ...
      <!-- ========== 管理员注册表单 ========== -->
      <div class="login-card hidden" id="admin-register-form">
        <div class="form-group">
          <label class="form-label">手机号码</label>
        ...
        <div class="form-group">
          ...
          <input type="password" class="form-input" id="reg-password2"
                 placeholder="再次输入密码"
                 autocomplete="new-password" />
          <div class="form-error" id="reg-password2-error"></div>
        </div>

        <button class="btn btn-primary btn-block btn-lg" id="admin-register-btn">
          注册并登录
        </button>
        <div class="text-center mt-12">
          <span class="text-link" id="switch-to-login" style="font-size:13px;color:var(--primary);cursor:pointer">
            已有账号？去登录
          </span>
        </div>
      </div>
  ...
  let adminSubMode = 'login';     // 'login' or 'register'
  ...
  function switchMode(mode) {
    ...
    document.getElementById('admin-login-form').classList.toggle('hidden', mode !== 'admin' || adminSubMode !== 'login');
    document.getElementById('admin-register-form').classList.toggle('hidden', mode !== 'admin' || adminSubMode !== 'register');
    document.getElementById('user-form').classList.toggle('hidden', mode !== 'user');
  }

  // === 管理员登录/注册子模式切换 ===
  function switchAdminSubMode(subMode) {
    adminSubMode = subMode;
    document.getElementById('admin-login-form').classList.toggle('hidden', subMode !== 'login');
    document.getElementById('admin-register-form').classList.toggle('hidden', subMode !== 'register');
  }

  document.getElementById('switch-to-register').addEventListener('click', () => switchAdminSubMode('register'));
  document.getElementById('switch-to-login').addEventListener('click', () => switchAdminSubMode('login'));
  ...
  regPassword2Input.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') handleAdminRegister();
  });
  adminRegisterBtn.addEventListener('click', handleAdminRegister);
  ...
  async function handleAdminRegister() {
    ...
    if (!/^1[3-9]\d{9}$/.test(phone)) { phoneError.textContent = '手机号码格式不正确'; return; }
    if (!password) { passwordError.textContent = '请输入密码'; return; }
    if (password.length < 6) { passwordError.textContent = '密码至少6位'; return; }
    if (password !== password2) { password2Error.textContent = '两次密码不一致'; return; }

    adminRegisterBtn.disabled = true;
    adminRegisterBtn.textContent = '注册中...';

    try {
      const data = await adminRegister(phone, password, nickname);
      showToast(data.isNew ? '注册成功！欢迎成为管理员' : '注册成功！');
      setTimeout(() => navigate('#/dashboard'), 500);
    } catch (err) {
      showToast(err.message || '注册失败');
      adminRegisterBtn.disabled = false;
      adminRegisterBtn.textContent = '注册并登录';
    }
  }
